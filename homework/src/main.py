...
def main():
    ...
    mlflow.set_experiment("wine_quality_experiment") # nombre del modelo
    run_name = f"{args.model}_{uuid.uuid4().hex[:8]}" #nombre unico de la corrida con el uuid que es un id unico
    with mlflow.start_run(run_name=run_name):

        ## Log de los parametros generales y del tipo de modelo
        mlflow.log_param("file_path", FILE_PATH) # cual fue el dataframe
        mlflow.log_param("test_size", TEST_SIZE) # cual fue el test
        mlflow.log_param("random_state", RANDOM_STATE) # cual fue el random
        mlflow.log_param("model_type", args.model) # cial fueron los parametos
